...
class ChatThreadsAdapter(private val onThreadClick: (CaseDto) -> Unit) :
    ListAdapter<CaseDto, ChatThreadsAdapter.ThreadViewHolder>(ThreadDiffCallback()) {
    ...
    class ThreadViewHolder(view: View) : RecyclerView.ViewHolder(view) {
        ...
        fun bind(item: CaseDto, onThreadClick: (CaseDto) -> Unit) {
            tvTitle.text = item.intake?.title ?: "Untitled Case"
            tvDate.text = formatToTime(item.submittedAt)
            ...
            tvPreview.text = item.intake?.medications ?: "No messages yet"
        ...
        private fun formatToTime(dateString: String): String {
            return try {
    ...
    override fun onBindViewHolder(holder: ThreadViewHolder, position: Int) {
        holder.bind(getItem(position), onThreadClick)
    }

    class ThreadDiffCallback : DiffUtil.ItemCallback<CaseDto>() {
        override fun areItemsTheSame(oldItem: CaseDto, newItem: CaseDto): Boolean = oldItem.id == newItem.id
        override fun areContentsTheSame(oldItem: CaseDto, newItem: CaseDto): Boolean = oldItem == newItem
    }
}
